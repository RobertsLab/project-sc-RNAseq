...
```
```{r}
blast <- read.delim("https://github.com/RobertsLab/project-sc-RNAseq/files/9262721/CPbla_geneBYcellsexpressed.txt", sep = "\t")
```
...
```{r}
ggplot(bb, aes(x=num_cells_expressed)) + 
    geom_histogram() +
...
```
```{r}
bb$level <- factor(bb$level,levels = c("none", "low", "medium", "high"))
```



```{r}
ggplot(bb, aes(x = level, fill = annotation)) + 
  geom_bar()
```
...
```{r}
ggplot(bb, aes(x = annotation, fill = level)) + 
  geom_bar()
```
...
```{r}
full_join(gast, blast, by = "id")
```

```{r}
lc <- full_join(gast, blast, by = "id")
```
